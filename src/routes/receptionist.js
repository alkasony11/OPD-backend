...
// Get list of doctors (for queue selection)
router.get('/doctors', authMiddleware, receptionistMiddleware, async (req, res) => {
  try {
    const doctors = await User.find({ role: 'doctor' }).select('name doctor_info');
    res.json({ doctors });
  } catch (error) {
...
router.post('/appointments', authMiddleware, receptionistMiddleware, async (req, res) => {
  try {
    ...
    // Check Age
    let patientAge = 0;
    let actualPatientName = patient.name;

    if (familyMemberObjectId) {
      const fm = await FamilyMember.findById(familyMemberObjectId);
      if (fm) {
        patientAge = fm.age;
        actualPatientName = fm.name;
      }
    } else {
      patientAge = patient.age || 0;
      if (!patientAge && patient.dob) {
        const diff = Date.now() - new Date(patient.dob).getTime();
        patientAge = Math.floor(diff / (1000 * 60 * 60 * 24 * 365.25));
      }
    }

    if (patientAge >= 60) isPriority = true;

    // Check Symptoms
    const urgentKeywords = ['chest pain', 'bleeding', 'severe', 'breathing', 'unconscious', 'emergency', 'heart attack', 'stroke', 'high fever'];
    if (symptoms && urgentKeywords.some(kw => String(symptoms).toLowerCase().includes(kw))) {
      isPriority = true;
    }
