...
Hello ${appointmentToken.patient_name},
...
`;
      ...
      // Use patient's registered phone number
      const recipientPhone = req.patient.phone;
      if (recipientPhone) {
        console.log(`Attempting to send WhatsApp to ${recipientPhone}`);
        // don't await so we don't block the response
        sendWhatsAppMessage(recipientPhone, whatsappMsg).then(res => {
...
router.get('/appointments', authMiddleware, patientMiddleware, async (req, res) => {
  try {
    ...
    // Calculate dynamic wait times for active appointments TODAY only
    const enhancedList = await Promise.all(appointmentList.map(async (apt) => {
      // Check if appointment is active and for today
      const now = new Date();
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      ...
      // Only show live ML wait time for TODAY's appointments
      const appointmentDate = new Date(apt.appointmentDate);
      appointmentDate.setHours(0, 0, 0, 0);
      const isToday = appointmentDate >= today && appointmentDate < tomorrow;

      if (['booked', 'confirmed', 'in_queue'].includes(apt.status) && isToday) {
        // Get live prediction only for today
        const prediction = await QueueService.calculateWaitTime(apt.tokenNumber, apt.doctorId, apt.appointmentDate, apt.sessionType);

        // Update the estimated wait time in the response
        apt.liveWaitTime = prediction.predictedWait;
        apt.tokensAhead = prediction.tokensAhead;
        apt.avgDrTime = prediction.avgConsultationTime;

        return {
          ...apt,
          estimatedWaitTime: prediction.predictedWait, // Override with live data
          tokensAhead: prediction.tokensAhead,
          doctorAvgTime: prediction.avgConsultationTime,
          isFutureAppointment: false
        };
      }

      // For future appointments, flag them and don't run ML predictions
      const isFuture = appointmentDate > today;
      if (['booked', 'confirmed'].includes(apt.status) && isFuture) {
        return {
          ...apt,
          isFutureAppointment: true,
          estimatedWaitTime: null // Clear the stale ML prediction
        };
      }
    ...
    res.json({ appointments: enhancedList });
  } catch (error) {
...
router.get('/appointments/:appointmentId/queue-position', authMiddleware, patientMiddleware, async (req, res) => {
  try {
    ...
    if (!['booked', 'confirmed', 'in_queue'].includes(appointment.status)) {
      return res.json({
        appointmentId: appointment._id,
    ...
    // If appointment is for a future date (not today), don't show live queue data
    ...
    // Get prediction based on appointment date (future or today)
    const prediction = await QueueService.calculateWaitTime(appointment.token_number, appointment.doctor_id._id, appointment.booking_date, appointment.session_type);

    // Find current appointment position in queue
    const queuePosition = prediction.tokensAhead + 1;
    const adjustedWaitTime = prediction.predictedWait;
    const avgConsultationTime = prediction.avgConsultationTime;
