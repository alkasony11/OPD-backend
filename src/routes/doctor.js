...
router.post('/consultation/start', authMiddleware, doctorMiddleware, async (req, res) => {
  try {
    ...
    res.json({ message: 'Consultation started', tokenId: token._id, startTime: token.startTime });
  } catch (error) {
...
router.post('/consultation/complete', authMiddleware, doctorMiddleware, async (req, res) => {
  try {
    ...
    // Fetch the token first to get startTime
    const currentToken = await Token.findOne({ _id: tokenId, doctor_id: req.doctor._id });
    if (!currentToken) return res.status(404).json({ message: 'Appointment not found' });

    let actualDuration = 0;
    if (currentToken.startTime) {
      actualDuration = (endTime - new Date(currentToken.startTime)) / 60000; // minutes
    } else {
      // Fallback if startTime wasn't captured (legacy or bug), assume default or slot duration
      // Here we will just use 0 or handle it gracefully.
      // But per instructions we should try to capture it.
      // If manually completing without start, maybe allow it but duration is 0.
      actualDuration = 15; // default fallback
    }

    const update = {
      status: 'consulted',
      consultation_completed_at: endTime,
      endTime: endTime,
    ...
    // AI Loop: Update doctor stats
    const doctor = await User.findById(req.doctor._id);
    if (doctor) {
      // Initialize if undefined (for legacy docs)
      if (typeof doctor.doctor_info.totalConsultations !== 'number') doctor.doctor_info.totalConsultations = 0;
      if (typeof doctor.doctor_info.totalConsultationTime !== 'number') doctor.doctor_info.totalConsultationTime = 0;

      doctor.doctor_info.totalConsultations += 1;
      doctor.doctor_info.totalConsultationTime += actualDuration;
      doctor.doctor_info.avgConsultationTime = doctor.doctor_info.totalConsultationTime / doctor.doctor_info.totalConsultations;

      await doctor.save();
    }

    res.json({ message: 'Consultation completed', actualDuration, newAvgTime: doctor?.doctor_info?.avgConsultationTime });
  } catch (error) {
