...
router.post('/ml/train-model', adminMiddleware, async (req, res) => {
  try {
    console.log('🔄 Admin requested ML model training...');

    const MLWaitTimePredictor = require('../services/mlWaitTimePredictor');
    const mlPredictor = new MLWaitTimePredictor();

    const success = await mlPredictor.trainModel();

    if (success) {
      const modelInfo = await mlPredictor.getModelInfo();
      res.json({
      ...
      res.status(500).json({
        message: 'ML model training failed - not enough data or training error'
      });
    }
  } catch (error) {
    console.error('ML training error:', error);
    res.status(500).json({ message: 'Server error during ML training' });
  }
});

// Get ML model information
router.get('/ml/model-info', adminMiddleware, async (req, res) => {
  try {
    const MLWaitTimePredictor = require('../services/mlWaitTimePredictor');
    const mlPredictor = new MLWaitTimePredictor();
    ...
    console.error('Get ML model info error:', error);
    res.status(500).json({ message: 'Server error' });
  }
});

// Retrain ML model
router.post('/ml/retrain-model', adminMiddleware, async (req, res) => {
  try {
    console.log('🔄 Admin requested ML model retraining...');

    const MLWaitTimePredictor = require('../services/mlWaitTimePredictor');
    const mlPredictor = new MLWaitTimePredictor();

    const success = await mlPredictor.retrainModel();
...
// Test ML prediction for a specific appointment
router.post('/ml/test-prediction', adminMiddleware, async (req, res) => {
  try {
    const { tokenNumber, doctorId, appointmentDate, timeSlot, sessionType, symptoms } = req.body;

    if (!tokenNumber || !doctorId) {
      return res.status(400).json({ message: 'tokenNumber and doctorId are required' });
    }
    ...
    res.json({
      prediction,
      input: { tokenNumber, doctorId, appointmentDate, timeSlot, sessionType, symptoms }
    });
  } catch (error) {
    console.error('ML test prediction error:', error);
    res.status(500).json({ message: 'Server error during prediction test' });
  }
});
