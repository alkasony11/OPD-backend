...
const userSchema = new mongoose.Schema({
  ...
  doctor_info: {
    ...
    totalConsultationTime: { type: Number, default: 0 }, // minutes
    consultation_fee: {
      type: Number,
      default: 500
    },
...
// appointmentSchema.index({ tokenNumber: 1 }); // Removed duplicate index
...
const tokenSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['booked', 'in_queue', 'in_progress', 'consulted', 'cancelled', 'missed', 'referred'],
    default: 'booked'
  },
  estimated_wait_time: {
    type: Number,
...
tokenSchema.index({ doctor_id: 1, booking_date: 1, session_type: 1, token_number: 1 }, { unique: true });
